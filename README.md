...
# Quarto Desafio Tecnico - MBA IA Engenharia de Software - FullCycle
...
- **Video Worker** (FFmpeg + BullMQ) — consome jobs da fila, processa vídeos com FFmpeg, atualiza DB e storage.
- **Object Storage** (MinIO/S3) — armazena arquivos de vídeo originais, versões processadas e thumbnails.
- **Message Queue** (Redis + BullMQ) — fila de processamento de vídeos entre API e Worker.
...
| MinIO (Object Storage) | http://localhost:9001 (console — user/senha: `streamtube`/`streamtube123`) |
...
**Todas as fases (01–07) estão concluídas** — backend NestJS + frontend Next.js + video-worker.
...
### Vídeos e Canal (Fases 03–04)

- **Upload em duas etapas:** `POST /videos/upload/initiate` (cria DRAFT + URL pré-assinada MinIO) → PUT direto do browser → `POST /videos/upload/confirm` (enfileira job BullMQ).
- **Video Worker** (`video-worker/`): consome jobs BullMQ, transcodifica com FFmpeg, grava versão processada no MinIO e atualiza o status para `ready`.
- **Gerenciamento:** listar vídeos do canal (`GET /videos/channel/me`), publicar (`PATCH /videos/:slug/publish`), editar metadados, excluir.
- **Página pública do canal** (`/channels/:nickname`): lista de vídeos publicados, contagem de inscritos.
- **Studio** (`/studio`): grid de todos os vídeos do canal com status e ações de gerenciamento.

### Página de Visualização (Fase 05)

- Player HTML5 via presigned URL (`GET /videos/:slug/stream`).
- Contador de visualizações (`POST /videos/:slug/view`).
- Botão de **download** do vídeo original via presigned URL com `Content-Disposition: attachment`.
- Sugestões de vídeos na sidebar (`GET /videos/:slug/suggestions`).

### Interações Sociais (Fase 06)

- **Likes/dislikes em vídeos** (`GET/POST/DELETE /videos/:slug/like`): atualização otimista no frontend.
- **Comentários** (`GET /videos/:slug/comments`, `POST /videos/:slug/comments`): suporte a respostas aninhadas (um nível), exclusão pelo autor.
- **Likes/dislikes em comentários** (`GET/POST/DELETE /comments/:id/like`, `POST /comments/:id/dislike`): componente `CommentLikeBar` client-side.
- **Inscrições em canais** (`GET/POST/DELETE /channels/:nickname/subscribe`): botão `SubscribeButton` com atualização otimista.

### Home e Busca (Fase 07)

- **Home page** (`/`): grid paginado de vídeos públicos com suporte a busca por título (`?q=`) e filtro por categoria.
- **Header global** com logo, campo de busca e links de autenticação.
- **Categorias** (`GET /categories`): lista de categorias disponíveis para filtro.
